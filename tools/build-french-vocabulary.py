#!/usr/bin/env python3
"""
Builds french-vocabulary.json from:
  1. kaikki.org per-POS JSONL files (French words from English Wiktionary)
  2. hermitdave/FrequencyWords for frequency ranking

Run:
  python3 tools/build-french-vocabulary.py [--out language-data/french-vocabulary.json] [--max 10000]
...
SKIP_PREFIXES = (
    ...
    'first/', 'second/', 'third/',          # e.g. "first/second-person"
    # Alternates / errors
...
def extract_entry(raw: dict, kaikki_pos: str) -> dict | None:
    ...
    # Skip multiword expressions (contains space) except for common verb phrases
    if ' ' in word:
        return None

    # Find the first real English gloss
    english = None
...
def stream_kaikki(pos: str, freq: dict[str, int]) -> dict[str, dict]:
    ...
    seen: dict[str, dict] = {}   # word → best entry so far
...
def build(out_path: str, max_words: int) -> None:
    ...
    for pos in ('noun', 'adj', 'adv', 'verb'):
        ...
        # Word-level dedup across POS: first wins
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build French vocabulary JSON')
    parser.add_argument('--out', default='language-data/french-vocabulary.json',
                        help='Output file (default: language-data/french-vocabulary.json)')
