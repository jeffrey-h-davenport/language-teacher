#!/usr/bin/env python3
"""
Builds czech-vocabulary.json from:
  1. kaikki.org per-POS JSONL files (Czech words from English Wiktionary)
  2. hermitdave/FrequencyWords for frequency ranking

Run:
  python3 tools/build-czech-vocabulary.py [--out language-data/czech-vocabulary.json] [--max 10000]
...
# Gloss prefixes that indicate an inflected/derived form, not a real lemma.
# Czech has 7 cases so inflection descriptions are common in kaikki data.
SKIP_PREFIXES = (
    # Generic inflection markers
    'inflection of', 'conjugate of', 'form of',
    'plural of', 'singular of',
    # Case forms (Czech: nominative/genitive/dative/accusative/vocative/locative/instrumental)
    'nominative of', 'genitive of', 'dative of', 'accusative of',
    'vocative of', 'locative of', 'instrumental of',
    'genitive plural of', 'dative plural of', 'accusative plural of',
    'nominative plural of', 'locative plural of', 'instrumental plural of',
    # Gender/number forms
    'feminine of', 'masculine of', 'neuter of',
    'feminine singular of', 'feminine plural of',
    'masculine singular of', 'masculine plural of',
    'neuter singular of', 'neuter plural of',
    'female equivalent of', 'male equivalent of',
    # Verbal forms
    'past participle of', 'present participle of',
    'past passive participle of', 'verbal noun of',
...
def extract_gender(entry: dict) -> str | None:
    ...
    Czech has masculine-animate, masculine-inanimate, feminine, neuter —
    both masculine subtypes are collapsed to 'masculine'.
    """
...
def extract_plural(entry: dict) -> str | None:
    """Return the nominative plural form, if present."""
    for form in entry.get('forms', []):
        ...
        if 'plural' in tags and 'nominative' in tags:
            ...
            if val and val != '-':
                return val
    # Fall back to any plural-tagged form
    for form in entry.get('forms', []):
        tags = form.get('tags', [])
        if 'plural' in tags:
            val = form.get('form', '').strip()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build Czech vocabulary JSON')
    parser.add_argument('--out', default='language-data/czech-vocabulary.json',
                        help='Output file (default: language-data/czech-vocabulary.json)')
